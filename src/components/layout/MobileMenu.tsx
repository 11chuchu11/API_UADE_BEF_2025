		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent
				...
				<div className="flex items-center justify-between">
					<span className="text-base xs:text-lg sm:text-xl font-semibold truncate">{title}</span>
					<SheetClose asChild>
						<HamburgerButton
							open
				...
				<div className="mt-3 xs:mt-4 flex-1 overflow-y-auto overscroll-contain">
						...
						<nav className="flex flex-col items-center gap-10 xs:gap-12">
							{items.map(item => (
								<SheetClose key={item.to} asChild>
									<Link to={item.to} className="text-xl sm:text-2xl font-semibold tracking-wide hover:opacity-90">
										{item.label}
									</Link>
								</SheetClose>
							))}
					...
					<div className={cn("mt-8 flex flex-col items-center gap-3 xs:gap-4",mode!=='admin' ? 'xs:mt-10 mt-8' : 'mt-0')}>
						{mode === 'admin' ? (
							<>
								<Button
									type="button"
				...
				<div className="mt-4 sm:mt-6 pb-6 pb-[env(safe-area-inset-bottom)] flex items-center justify-center gap-10 xs:gap-12 text-current/90">
					<a className="hover:opacity-80 text-2xl xs:text-3xl" href="#" aria-label="Instagram">
						<FiInstagram />
					</a>
					<a className="hover:opacity-80 text-2xl xs:text-3xl" href="#" aria-label="Facebook">
						<FiFacebook />
					</a>
					<a className="hover:opacity-80 text-2xl xs:text-3xl" href="#" aria-label="YouTube">
						<FiYoutube />
					</a>
				</div>
			</SheetContent>
