...
const Header: React.FC = () => {
	...
	const { headerAtTop, headerScrolled, titleAtTop, titleScrolled, center, action } = useHeaderVariant()
	...
	const handleLogout = () => {
		deleteCookie(COOKIES.IS_LOGGED.NAME)
		navigate('/')
	}
	...
	return (
		<header
			className={cn(
				'w-full fixed z-50 transition-all duration-1000 ease-in-out text-white lg:px-20 xs:px-0 py-4',
				scrolled ? headerScrolled : headerAtTop
			)}>
			<div className="flex h-12 items-center justify-between px-6 lg:px-20">
				<h1 className={cn('text-xl sm:text-xl font-bold transition-colors duration-1000 ease-in-out', scrolled ? titleScrolled : titleAtTop)}>
					{texts.header.h1}
				</h1>
				...
				<div className="flex items-center">
					...
					<HamburgerButton
						...
						className={cn('lg:hidden', scrolled ? 'text-primary-foreground' : isAdmin ? 'text-primary' : 'text-primary-foreground')}
					/>
				</div>
			</div>
			...
			{action.kind === 'login' && <LoginDialog open={loginOpen} onOpenChange={setLoginOpen} />}
		</header>
	)
}
