...
// --- NUEVO: utilidades para comparar horarios de forma robusta ---
...
export function ScheduleForm({ onCancel, onSuccess }: Props) {
  ...
  // --- NUEVO: horarios ya tomados para el día seleccionado (requested + confirmed) ---
  ...
  // Slots disponibles (filtra hoy por hora mínima y SIEMPRE elimina los ya tomados)
  const availableSlots = useMemo(() => {
    ...
    // --- NUEVO: no mostrar los horarios ya reservados ---
    list = list.filter(({ h, m }) => !bookedSet.has(toMinutes(h, m)));
  ...
  // --- NUEVO: si cambio de día y el horario elegido queda inválido, lo limpio ---
  useEffect(() => {
    if (!time) return;
    ...
    >
      ...
      <Card className="h-full rounded-2xl p-4 sm:p-5 md:p-6 shadow-sm">
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.apellido && (
              <p className="mt-1 text-xs text-rose-600">{errors.apellido}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.insurance && (
              <p className="mt-1 text-xs text-rose-600">{errors.insurance}</p>
            )}
          </div>
        </div>
      </Card>
            ...
            <div className="w-full flex justify-center">
              <Calendar
                ...
                classNames={{
                  ...
                  head_cell:
                    "text-center text-muted-foreground font-normal text-xs",
            ...
            <div className="mt-2">
              ...
              >
                ...
                >
                  <SelectValue
                    placeholder={
                      noSlotsForSelectedDay ? "Sin turnos ese día" : "Seleccionar"
                    }
                  />
                </SelectTrigger>
      ...
      <div className="md:col-span-2 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        ...
        <Button
          ...
          variant="brand"
          size="pill"
          disabled={submitting || !!noSlotsForSelectedDay}
          className="uppercase tracking-wide w-full sm:w-auto"
        >
          Solicitar Reserva
        </Button>
